...
func Init() {
	commandHints = map[string][]string{
		"allow": {
			"Grant additional access to a team or role using `torus allow`",
		},
		"context": {
			"Your linked organization and project are found in .torus.json after running `torus link`",
		},
		"deny": {
			"Restrict access for a team or role using `torus deny`",
		},
		"invites approve": {
			"Approve multiple invites quickly with `torus worklog resolve`",
		},
		"invites send": {
			"Invite another user to collaborate in your organization with `torus invites send`",
		},
		"link": {
			"Define an organization and project for your current working directory using `torus link`",
		},
		"ls": {
			"Explore the objects and secrets set for your org through `torus ls`",
		},
		"path": {
